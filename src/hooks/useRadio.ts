import { inject, InputHTMLAttributes, provide, Events, computed, toRefs, Ref } from 'vue';
...
export interface AppRadioProps extends Omit<InputHTMLAttributes, 'class'> {
  label?: string;
  name?: string;
  id?: string;
...
export const useRadioGroup = (
  ...
  emit: (event: 'update:modelValue', ...args: (string | number | boolean)[]) => void
) => {
  // to keep reactivity, destructure using toRefs
  const { name, disabled, modelValue, defaultValue } = toRefs(props);

  // set up controlled and uncontrolled component...
  const [realVal, setRealVal] = useControlled(modelValue, defaultValue);
...
export const useRadio = <T extends AppRadioProps>(
  ...
  const groupContext = inject<INJECT_KEY_PROP>(INJECT_KEY, undefined);
  ...
  const id = computed(() => String(props.id || props.value));
